...
void main() {
  ...
  group('_completeSmsLoginFromResponseData clears old password creds', () {
    test('removes hutUsername/hutPassword and marks hutAuthMethod=sms', () async {
      ...
      expect(prefs.getString('hutToken'), 'new-id');
  ...
  group('checkTokenValidity SMS branch', () {
    ...
    test('returns true for a fresh JWT', () async {
      final futureExp = (DateTime.now().millisecondsSinceEpoch ~/ 1000) + 3600;
      SharedPreferences.setMockInitialValues({
        'hutAuthMethod': kHutAuthMethodSms,
        'hutToken': _buildJwt({'exp': futureExp}),
      });
      expect(await HutUserApi().checkTokenValidity(), isTrue);
    });

    test('falls back to SMS when hutAuthMethod missing and no hutUsername',
        () async {
      final futureExp = (DateTime.now().millisecondsSinceEpoch ~/ 1000) + 3600;
      SharedPreferences.setMockInitialValues({
        'hutToken': _buildJwt({'exp': futureExp}),
