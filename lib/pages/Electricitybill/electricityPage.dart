...
class _ElectricityPageState extends State<ElectricityPage> {
  ...
  Future<bool> chargeMoney() async {
    ...
    if(firstCheck!=true){
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('未知错误')),
      );
      return false;
    }
    //创建订单
    Map _orderInfo =await electricityApi.createOrder(_roomToChargeId,_payment,_roomToChargeName);
    print("the orderInfo::::$_orderInfo");
    //完成充值
    electricityApi.finishRecharge(_orderInfo['payorderno'], _payment,_roomToChargeName);
    //充值成功
    getNewRoomInfo(_roomToChargeId);
    getBalance();
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(content: Text('充值成功')),
    );
    return true;
  }
  @override
  Widget build(BuildContext context) {
    return Scaffold(
        ...
        child: ListView(
          children: [
            Container(
                ...
                child: EnhancedFutureBuilder(future: getHisRoomInfo(),
                  ...
                  children: [
                    ...
                    TextButton(onPressed: () async {
                      ...
                      if(_paymentController.text.isEmpty){
                        ScaffoldMessenger.of(context).showSnackBar(
                          SnackBar(content: Text('充值金额不能为空')),
                        );
                        return;
                      }
                      // 层级校验 2: 数字格式校验
                      ...
                      if (amount == null) {
                        ScaffoldMessenger.of(context).showSnackBar(
                          SnackBar(content: Text('请输入有效的数字格式')),
                        );
                        return;
                      }
                      ...
                      if (amount <= 0) {
                        ScaffoldMessenger.of(context).showSnackBar(
                          SnackBar(content: Text('金额必须大于0元')),
                        );
                        return;
                      }
                      ...
                      if (!decimalPattern.hasMatch(_paymentController.text)) {
                        ScaffoldMessenger.of(context).showSnackBar(
                          SnackBar(content: Text('最多支持两位小数')),
                        );
                        return;
                      }
      ...
      Container(
        ...
        child: ListTile(
          leading: Icon(Ionicons.grid_outline, color: Theme.of(context).primaryColor),
          title: Text(
            "更改充值房间",
            style: TextStyle(fontWeight: FontWeight.w500),
          ),
          trailing: isRoomLoading ? LoadingAnimationWidget.inkDrop(color: Theme.of(context).primaryColor, size: 20) : Icon(Icons.chevron_right, color: Colors.grey),
          onTap: () async {
            setState(() {
              isRoomLoading = true;
            });
    ...
    showCupertinoModalBottomSheet(
      ...
      builder: (context) => StatefulBuilder(
        ...
        builder: (context, setState) {
          // 根据搜索词过滤房间列表
          final filteredRooms = RoomList.where((room) {
                  ...
                  Container(
                    ...
                    child: ListView.builder(
                      ...
                      itemBuilder: (BuildContext context, int index) {
                        ...
                        return ListTile(
                          leading: Icon(Ionicons.shapes_outline, color: Colors.blue),
                  ...
                  children: [
                    ...
                    TextField(
                      ...
                      decoration: InputDecoration(
                        ...
                        prefixIcon: Icon(Ionicons.alert_circle_outline, color: Colors.orange),
                      ),
                    ...
                    SizedBox(
                      ...
                      child: ElevatedButton(
                        onPressed: () async {
                          String alertCount = deviceCodeController.text.trim();
                          ...
                          prefs.setDouble('enableBill', double.parse(alertCount));
                          Navigator.pop(context);
                        ...
                        child: Text(isEnable?'更改预警':'设置预警', style: TextStyle(fontSize: 18)),
                      ),
                    ),
                    Visibility(
                      visible: isEnable,
                        ...
                        children: [
                          ...
                          SizedBox(
                            ...
                            child: ElevatedButton(
                              ...
                              child: Text('关闭预警', style: TextStyle(fontSize: 18)),
                            ),
                          ),
                        ],
                      ),
                      ...
                      child: Container(
                        ...
                        child: Column(
                          ...
                          children: [
                            ...
                            SizedBox(height: 8),
                            Text('当检测到${setRoomName}的电费小于预警值后，将会在进入超级工大时进行提醒'),
                          ],
                        ),
                      ),
                    ),
