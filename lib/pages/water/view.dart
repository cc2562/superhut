...
extension SizeExtension on num {
  double get w => ScreenUtils.length(vertical: toDouble(), horizon: toDouble() * 0.4);
  double get sp => ScreenUtils.length(vertical: toDouble(), horizon: toDouble() * 0.5);
}
...
class _FunctionHotWaterPageState extends State<FunctionHotWaterPage> {
  ...
  Widget waterStatusWidget() {
    return GetBuilder<FunctionHotWaterLogic>(builder: (logic) {
      return Container(
  ...
  Widget deviceDrinkRowBtnWidget() {
    return Padding(
      ...
      child: GetBuilder<FunctionHotWaterLogic>(builder: (logic) {
        ...
        // 显示当前选择的设备
        String deviceName = logic.state.deviceList[logic.state.choiceDevice.value]["posname"];
        return GestureDetector(
          onTap: () {
            _showDeviceSelectionDialog(context);
          },
                      ...
                      itemBuilder: (context, index) {
                        ...
                        return ListTile(
                          ...
                          trailing: logic.state.choiceDevice.value == index
                              ? Icon(Ionicons.checkmark_circle, color: Colors.orange)
                              : null,
                          onTap: () {
                            if (logic.state.waterStatus.value) {
...
class _BubbleAnimationState extends State<BubbleAnimation> with SingleTickerProviderStateMixin {
  ...
  Widget build(BuildContext context) {
    return Stack(
      children: [
        for (var bubble in bubbles) bubble,
      ],
    );
  }
}
