    ...
    if (url.contains(_targetDomain) && url.contains('token=')) {
      try {
        ...
        if (fragment.isNotEmpty && fragment.contains('token=')) {
          ...
          int tokenStartIndex = fragment.indexOf('token=') + 6; // 'token='.length
        ...
        children: [
          InAppWebView(
            initialUrlRequest: URLRequest(
              url: WebUri('$_initialUrl?idToken=${widget.initialIdToken}&service=https%3A%2F%2Fjwxtsj.hut.edu.cn%2Fnjwhd%2FloginSso&token=${widget.initialIdToken}'),
              headers: {
